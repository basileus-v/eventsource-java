...
public class EventSourceClientTest {
  ...
  @Test
  @Ignore
  // Because of https://github.com/joewalnes/webbit/issues/29
  public void reconnectsIfServerGoesDownAfterConnectionEstablished() throws Exception {
    ...
    webServer.add("/es/.*", new org.webbitserver.EventSourceHandler() {
    ...
    eventSource =
        new EventSource(new EventSourceClient(),
                        100,
                        URI.create("http://localhost:59504/es/hello"),
  ...
  private void assertSentAndReceived(final List<String> messages) throws IOException, InterruptedException {
    ...
    assertTrue("Didn't get all messages", messageCountdown.await(100000, TimeUnit.MILLISECONDS));
  }
  ...
  private void startClient(final List<String> expectedMessages,
    ...
    eventSource =
        new EventSource(new EventSourceClient(),
                        ...
                        URI.create("http://localhost:59504/es/hello?echoThis=yo"),
                        new EventSourceHandler() {
                          int n = 0;
